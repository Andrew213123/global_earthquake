...
function GeocoderViewModel(options) {
  if (!defined13(options) || !defined13(options.scene)) {
    throw new DeveloperError11("options.scene is required.");
  }
  if (defined13(options.geocoderServices)) {
  ...
  this._searchCommand = createCommand_default(function(geocodeType) {
    ...
    if (defined13(that._selectedSuggestion)) {
...
function computeFlyToLocationForCartographic(cartographic, terrainProvider) {
  const availability = defined13(terrainProvider) ? terrainProvider.availability : void 0;
  if (!defined13(availability)) {
...
async function geocode(viewModel, geocoderServices, geocodeType) {
  ...
  for (i = 0; i < geocoderServices.length; i++) {
    ...
    if (defined13(result) && result.state === "fulfilled" && result.value.length > 0) {
      break;
    }
  }
  if (viewModel._wasGeocodeCancelled) {
  ...
  if (result.state === "fulfilled" && defined13(geocoderResults) && geocoderResults.length > 0) {
...
function updateCredit(viewModel, credit) {
  if (defined13(credit) && !viewModel._scene.isDestroyed() && !viewModel._scene.frameState.creditDisplay.isDestroyed()) {
  ...
  for (const service of viewModel._geocoderServices) {
    ...
    if (newResults.length > 0) {
      ...
      newResults.forEach((result) => {
        ...
        useDefaultCredit = useDefaultCredit && !defined13(credits);
...
function Geocoder(options) {
  if (!defined14(options) || !defined14(options.container)) {
    throw new DeveloperError12("options.container is required.");
  }
  if (!defined14(options.scene)) {
...
import { defined as defined15, DeveloperError as DeveloperError13 } from "@cesium/engine";
...
function NavigationHelpButton(options) {
  if (!defined19(options) || !defined19(options.container)) {
...
import { Check as Check4, defined as defined21 } from "@cesium/engine";

// packages/widgets/Source/I3SBuildingSceneLayerExplorer/I3SBuildingSceneLayerExplorerViewModel.js
import { defined as defined20 } from "@cesium/engine";
...
function I3SBuildingSceneLayerExplorerViewModel(i3sProvider) {
  ...
  for (let i = 0; i < sublayers.length; i++) {
    ...
    if (defined20(topLayer) && (isOverview(topLayer) || !defined20(this.viewModel.defaultLayer) && isFullModel(topLayer))) {
...
function I3SBuildingSceneLayerExplorer(containerId, i3sProvider) {
  ...
  if (defined21(viewModel.defaultLayer)) {
